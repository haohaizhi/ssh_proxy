...
#define KEY_PATH "/home/sshproxy/rsa_admin_key"
...
#include <argp.h>
#endif
//通过 utmp.h 提供的函数和数据结构，可以读取和操作用户信息
#ifdef HAVE_UTMP_H
#include <utmp.h>
#endif
...
#ifdef HAVE_ARGP_H
const char *argp_program_version = "WY command proxy "
SSH_STRINGIFY("V1.0.0");
const char *argp_program_bug_address = "<hongqiang@ieucd.com>";

/* Program documentation. */
static char doc[] = "vtysh_proxy -- A generic command line agent";
...
static void handle_session(ssh_event event, ssh_session session) {
    ...
    struct ssh_channel_callbacks_struct channel_cb = {
        ...
        //当客户端希望与远程主机建立一个交互式 shell 会话时，会发送一个 shell 请求
        .channel_shell_request_function = shell_request,
        //当客户端希望远程主机执行特定的命令，并获取其执行结果时，会发送一个执行命令的请求
